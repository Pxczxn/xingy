<template>
  <div class="page-container page-container--scroll">
    <header class="report-intro">
      <div class="report-intro-copy">
        <h1>定时任务</h1>
        <p>查看调度执行统计与趋势，掌握任务成功率与近期运行情况。</p>
      </div>
      <img class="report-intro-illustration" src="/images/report-analytics-charts.png" alt="图表与 KPI 分析插画" />
    </header>
    ...
    <n-modal v-model:show="showLogModal" :title="logModalTitle" preset="card" style="width: 1000px">
      <n-data-table
        v-bind="tableListProps(logScroll)"
        :columns="logColumns"
        :data="logData"
        :loading="logLoading"
...
const columns: DataTableColumns<SysJob> = [
  { title: 'ID', key: 'id', ...colLayout('id') },
  { title: '任务名称', key: 'jobName', ...colLayout('name') },
  { title: '任务组名', key: 'jobGroup', ...colLayout('slug') },
  { title: '调用目标', key: 'invokeTarget', ...colLayout('description') },
  { title: 'cron表达式', key: 'cronExpression', ...colLayout('slug') },
  ...
  { title: '操作', key: 'actions', ...colLayout('action4'), render(row) {
    const buttons = []
...
const logColumns: DataTableColumns<SysJobLog> = [
  ...
  { title: '任务名称', key: 'jobName', ...colLayout('name') },
  { title: '调用目标', key: 'invokeTarget', ...colLayout('description') },
  { title: '日志信息', key: 'jobMessage', ...colLayout('summary') },
  { title: '状态', key: 'status', ...colLayout('status'), render(row) {
...
@media (max-width: 800px) {
  .report-intro-illustration { display: none; }
}
